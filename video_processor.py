...
from utils import run_ffmpeg, get_video_duration
...
def preprocess_clip(input_path: str, output_path: str, duration: float) -> str:
    ...
    ffmpeg_cmd = [
        ...
        "-vf", f"setpts=PTS/{config.SPEED_RATE},scale=1080:1920,eq=contrast=1.1:saturation=1.15,unsharp=5:5:0.8,fps=30",
...
def join_clips(clip_paths: list, output_path: str) -> str:
    """
    多段视频顺序拼接
    ...
    """
    # 生成 concat 所需的文件列表
    concat_list_txt = os.path.join(config.TEMP_DIR, "concat_list.txt")
...
def slice_dedup_videos(dedup_videos: list, segment_duration: float) -> list:
    """
    ...
    for vid_path in dedup_videos:
        vid_duration = get_video_duration(vid_path)
        ...
        for seg_idx in range(num_segments):
            ...
            out_path = os.path.join(config.TEMP_DIR, f"dedup_seg_{clip_idx}.mp4")
